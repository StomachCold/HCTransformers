...
def testCos(args,server,epoch,pretrained_weights,file=None):
    ...
    iter_num = 10000
    print(iter_num)
    file_path = pretrained_weights + 'test_224_{}_{}.hdf5'.format(epoch, args.checkpoint_key)
    if file is not None:
        file_path = file
    ...
    with open(pretrained_weights+'/test_log_{}_{}.txt'.format(server['dataset'],args.checkpoint_key), 'a+') as f:
        f.write(log_info)
